...
export function mapEvento(dto: EventResponseDTO): Evento {
  return {
    id: dto.id,
    nome: dto.name,
    descricao: dto.description ?? "",
    data: (dto.eventDate ?? "").slice(0, 10),
    categoria: inferCategoria(dto),
    ingressosDisponiveis: dto.availableTickets ?? 0,
    preco: dto.price ?? 0,
    imagemUrl: dto.imageUrl ?? null,
  };
}
